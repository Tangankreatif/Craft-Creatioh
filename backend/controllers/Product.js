...
export const updateProduct = async(req, res) => {
    try {
        ...
        const { namaproduk, deskripsi, harga } = req.body;
        if (req.role === "admin") {
            await Product.update({ namaproduk, deskripsi, harga }, {
                where: {
                    id: product.id
                }
            });
        } else {
            if (req.penjualId !== product.penjualId) return res.status(403).json({ msg: "Akses Terlarang" });
            await Product.update({ namaproduk, deskripsi, harga }, {
                where: {
                    [Op.and]: [{ id: product.id }, { penjualId: req.penjualId }]
                }
        ...
        res.status(200).json({ msg: "Product berhasil di perbaharui" });
    } catch (error) {
        res.status(500).json({ msg: error.message });
    }
}
